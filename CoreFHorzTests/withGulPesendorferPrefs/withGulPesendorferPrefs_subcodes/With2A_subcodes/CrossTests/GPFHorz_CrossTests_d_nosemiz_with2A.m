function output=GPFHorz_CrossTests_d_nosemiz_with2A(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)
% The Gul-Pesendorfer with2A cross tests, with a decision variable d: the same tests run on
% the two-endogenous-state model, pinning each GP 2A raw ({plain, DC2A, GI2A, DC2A+GI2A}) to
% its exponential donor via the lambdaGP=0 limit, plus the constant-temptation,
% shift-invariance, V_Jplus1 and sign tests. The hand-rolled brute-force test of the d donor
% is NOT repeated here: the main tier already pinned the temptation logic against the
% hand-rolls, and the 2A raws' new risk is indexing, which the lambda=0 and constant-v
% anchors catch.
% Note: the temptation fns deliberately have different parameters to the ReturnFns (no
% eta/varphi/phi1/phi2), so these tests also cover TemptationFnParamNames differing from
% ReturnFnParamNames.
...
% Test 4: age-dependent lambdaGP via V_Jplus1, at all four tiers.
% Test 5: sign check: V_GP <= V_standard everywhere.
...
TemptationFn_z=@(d,a1prime,a2prime,a1,a2,z,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension) GPTemptationFn_d_z_noe_nosemiz_with2A(d,a1prime,a2prime,a1,a2,z,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension);
TemptationFn_e=@(d,a1prime,a2prime,a1,a2,e,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension) GPTemptationFn_d_noz_e_nosemiz_with2A(d,a1prime,a2prime,a1,a2,e,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension);
TemptationFn_ze=@(d,a1prime,a2prime,a1,a2,z,e,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension) GPTemptationFn_d_z_e_nosemiz_with2A(d,a1prime,a2prime,a1,a2,z,e,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension);
TemptationFn_none=@(d,a1prime,a2prime,a1,a2,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension) GPTemptationFn_d_noz_noe_nosemiz_with2A(d,a1prime,a2prime,a1,a2,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension);
