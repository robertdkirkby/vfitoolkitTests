function output=GPFHorz_CrossTests_nod_nosemiz(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)
% The Gul-Pesendorfer cross tests (without d). Unlike the ambiguity-aversion bank, the
% lambdaGP=0 limiting case switches the temptation machinery OFF entirely, so limiting cases
% alone never exercise it; hence the hand-rolled brute-force recursions in tests 4 and 4b.
...
% Test 4: hand-rolled brute-force GP recursion (small grid, plain MATLAB loops, full-choice-set
%         most-tempting term) vs the toolkit at the plain and DC tiers. The model's most
%         tempting choice (the consumption splurge, low aprime) is far from the optimum, so a
%         DC implementation that wrongly took the temptation max over its restricted window
%         would show up here.
% Test 4b: hand-rolled GP grid-interpolation recursion (temptation max over the FINE grid via
%         its own two-stage refinement) vs the toolkit GI tier. First verified against the
%         standard GI solver at lambdaGP=0 to pin the hand-roll to the toolkit GI conventions.
...
TemptationFn_z=@(aprime,a,z,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension) GPTemptationFn_nod_z_noe_nosemiz(aprime,a,z,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension);
TemptationFn_e=@(aprime,a,e,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension) GPTemptationFn_nod_noz_e_nosemiz(aprime,a,e,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension);
TemptationFn_ze=@(aprime,a,z,e,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension) GPTemptationFn_nod_z_e_nosemiz(aprime,a,z,e,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension);
TemptationFn_none=@(aprime,a,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension) GPTemptationFn_nod_noz_noe_nosemiz(aprime,a,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension);
...
% V_j(a,z) = max_{a'} [ u + v + beta*E V_{j+1} ] - max_{a'} v, with BOTH maxes over the full
% choice set. Operation order matches the raws: max((U+T)+beta*EV) with the most-tempting
% term subtracted AFTER the max, so any difference beyond the ULP floor is a real discrepancy.
n_a_bf=51;
a_grid_bf=5*linspace(0,1,n_a_bf)'.^3;
pi_z_cpu=gather(pi_z);
z_grid_cpu=gather(z_grid);
U_j=zeros(n_a_bf,n_a_bf,n_z);
T_j=zeros(n_a_bf,n_a_bf,n_z);
V_bf=zeros(n_a_bf,n_z,N_j);
P_bf=zeros(n_a_bf,n_z,N_j);
for jj=N_j:-1:1
    for z_c=1:n_z
        for a_c=1:n_a_bf
            for ap_c=1:n_a_bf
                U_j(ap_c,a_c,z_c)=ReturnFn_z(a_grid_bf(ap_c),a_grid_bf(a_c),z_grid_cpu(z_c),Params.r,Params.w,Params.kappa_j(jj),Params.sigma,Params.agej(jj),Params.Jr,Params.pension);
                T_j(ap_c,a_c,z_c)=TemptationFn_z(a_grid_bf(ap_c),a_grid_bf(a_c),z_grid_cpu(z_c),Params.lambdaGP,Params.shiftGP,Params.r,Params.w,Params.kappa_j(jj),Params.sigma,Params.agej(jj),Params.Jr,Params.pension);
            end
        end
    end
    if jj==N_j
        EVterm=zeros(n_a_bf,1,n_z);
    else
        EVterm=reshape(V_bf(:,:,jj+1)*pi_z_cpu',[n_a_bf,1,n_z]); % E[V_{j+1}(a',z')|z], (aprime,1,z)
    end
    MostT=max(T_j,[],1); % full-choice-set most-tempting, (1,a,z)
    entireRHS=U_j+T_j+Params.beta*EVterm;
...
fprintf('Cross test 4 (z_noe, DC): toolkit vs hand-rolled brute force, this should be zero: %.3e \n',max(abs(gather(Ptk2(:))-P_bf(:))))
clear Vtk1 Vtk2 Ptk1 Ptk2

%% ==================== Cross test 4b: hand-rolled grid interpolation ====================
% Hand-rolled GP-GI on the small grid, replicating the toolkit GI conventions: midpoint from
% the coarse argmax of the tempted objective (clamped to [2,n_a-1]), fine window of
% 2*ngridinterp+3 points spanning the neighbouring coarse cells, EV linearly interpolated onto
% the fine grid; and (GP-specific, design decision D2) the most-tempting term is the max of v
% over the FINE grid, found by the same two-stage scheme around v's OWN coarse argmax.
% Step 1 pins the hand-roll to the toolkit GI conventions at lambdaGP=0 (against the standard
% GI solver); step 2 is then the real test of the GP-GI raws. V only (Policy layout under GI
% is checked by the GI-vs-DC+GI comparisons in the per-case subcodes).
n2short=5; % =ngridinterp used throughout the bank
n2long=2*n2short+3;
aprime_grid_bf=interp1(1:1:n_a_bf,a_grid_bf,linspace(1,n_a_bf,n_a_bf+(n_a_bf-1)*n2short))';
n_afine=length(aprime_grid_bf);
U_f=zeros(n_afine,n_a_bf,n_z);
T_f=zeros(n_afine,n_a_bf,n_z);
for lambdacase=1:2 % 1: lambdaGP=0 (validate hand-roll vs standard GI); 2: baseline lambdaGP (test GP-GI)
    ...
    V_bfGI=zeros(n_a_bf,n_z,N_j);
    for jj=N_j:-1:1
        for z_c=1:n_z
            for a_c=1:n_a_bf
                for ap_c=1:n_a_bf
                    U_j(ap_c,a_c,z_c)=ReturnFn_z(a_grid_bf(ap_c),a_grid_bf(a_c),z_grid_cpu(z_c),Params.r,Params.w,Params.kappa_j(jj),Params.sigma,Params.agej(jj),Params.Jr,Params.pension);
                    T_j(ap_c,a_c,z_c)=TemptationFn_z(a_grid_bf(ap_c),a_grid_bf(a_c),z_grid_cpu(z_c),lambdaval,Params.shiftGP,Params.r,Params.w,Params.kappa_j(jj),Params.sigma,Params.agej(jj),Params.Jr,Params.pension);
                end
                for ap_c=1:n_afine
                    U_f(ap_c,a_c,z_c)=ReturnFn_z(aprime_grid_bf(ap_c),a_grid_bf(a_c),z_grid_cpu(z_c),Params.r,Params.w,Params.kappa_j(jj),Params.sigma,Params.agej(jj),Params.Jr,Params.pension);
                    T_f(ap_c,a_c,z_c)=TemptationFn_z(aprime_grid_bf(ap_c),a_grid_bf(a_c),z_grid_cpu(z_c),lambdaval,Params.shiftGP,Params.r,Params.w,Params.kappa_j(jj),Params.sigma,Params.agej(jj),Params.Jr,Params.pension);
                end
            end
        end
        if jj==N_j
            EVterm=zeros(n_a_bf,n_z);
            EVinterp=zeros(n_afine,n_z);
        else
            EVterm=V_bfGI(:,:,jj+1)*pi_z_cpu'; % (aprime,z)
            EVinterp=interp1(a_grid_bf,EVterm,aprime_grid_bf); % (fine aprime,z)
        end
        % coarse stage of the tempted objective, and of the temptation alone
        RHS_c=U_j+T_j+Params.beta*reshape(EVterm,[n_a_bf,1,n_z]);
        [~,mi]=max(RHS_c,[],1);
        midpoint=max(min(mi,n_a_bf-1),2);
        [~,miT]=max(T_j,[],1);
        midpointT=max(min(miT,n_a_bf-1),2);
        for z_c=1:n_z
            for a_c=1:n_a_bf
                fidx=(midpoint(1,a_c,z_c)+(midpoint(1,a_c,z_c)-1)*n2short)+(-n2short-1:1:1+n2short)';
                Vtilde=max(U_f(fidx,a_c,z_c)+T_f(fidx,a_c,z_c)+Params.beta*EVinterp(fidx,z_c));
                fidxT=(midpointT(1,a_c,z_c)+(midpointT(1,a_c,z_c)-1)*n2short)+(-n2short-1:1:1+n2short)';
                MostTfine=max(T_f(fidxT,a_c,z_c));
                V_bfGI(a_c,z_c,jj)=Vtilde-MostTfine;
            end
        end
    end
    if lambdacase==1
        [VtkGI,PtkGI]=ValueFnIter_Case1_FHorz(n_d,n_a_bf,n_z,N_j,d_grid,a_grid_bf,z_grid,pi_z,ReturnFn_z,Params,DiscountFactorParamNames,[],vfstd3);
        ...
        [VtkGI,PtkGI]=ValueFnIter_Case1_FHorz(n_d,n_a_bf,n_z,N_j,d_grid,a_grid_bf,z_grid,pi_z,ReturnFn_z,Params,DiscountFactorParamNames,[],vfgp3);
        fprintf('Cross test 4b (z_noe, GI): toolkit GP-GI vs hand-rolled GP-GI, this should be zero: %.3e \n',max(abs(gather(VtkGI(:))-V_bfGI(:))))
    end
end
clear U_j T_j U_f T_f V_bf P_bf V_bfGI VtkGI PtkGI
