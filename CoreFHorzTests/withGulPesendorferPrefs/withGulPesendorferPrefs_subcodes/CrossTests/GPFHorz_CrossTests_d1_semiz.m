function output=GPFHorz_CrossTests_d1_semiz(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)
% The Gul-Pesendorfer semiz cross tests, with d1 (n_d/d_grid passed in are n_d_semiz/d_grid_semiz).
...
%         every GP semiz raw gets a limiting-case anchor.
...
%         fns coincide, must reproduce the (GPU-validated) nosemiz GP solve. At all four
%         tiers. This pins the semiz temptation machinery (per-d2 most-tempting collection,
%         fine-grid refinement under GI) against the known-good nosemiz family.
% Test 3: V_Jplus1 short solve vs full solve (lambdaGP=0.1), at all four tiers (runtime
%         coverage of the GP semiz raws' V_Jplus1 branches).
...
ReturnFn_z=@(d1,d2,aprime,a,semiz,z,r,w,kappa_j,sigma,agej,Jr,pension,eta,varphi,uempbenefit,searcheffortcost) ReturnFn_d1_z_noe_semiz(d1,d2,aprime,a,semiz,z,r,w,kappa_j,sigma,agej,Jr,pension,eta,varphi,uempbenefit,searcheffortcost);
ReturnFn_e=@(d1,d2,aprime,a,semiz,e,r,w,kappa_j,sigma,agej,Jr,pension,eta,varphi,uempbenefit,searcheffortcost) ReturnFn_d1_noz_e_semiz(d1,d2,aprime,a,semiz,e,r,w,kappa_j,sigma,agej,Jr,pension,eta,varphi,uempbenefit,searcheffortcost);
ReturnFn_ze=@(d1,d2,aprime,a,semiz,z,e,r,w,kappa_j,sigma,agej,Jr,pension,eta,varphi,uempbenefit,searcheffortcost) ReturnFn_d1_z_e_semiz(d1,d2,aprime,a,semiz,z,e,r,w,kappa_j,sigma,agej,Jr,pension,eta,varphi,uempbenefit,searcheffortcost);
ReturnFn_none=@(d1,d2,aprime,a,semiz,r,w,kappa_j,sigma,agej,Jr,pension,eta,varphi,uempbenefit,searcheffortcost) ReturnFn_d1_noz_noe_semiz(d1,d2,aprime,a,semiz,r,w,kappa_j,sigma,agej,Jr,pension,eta,varphi,uempbenefit,searcheffortcost);

TemptationFn_z=@(d1,d2,aprime,a,semiz,z,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit) GPTemptationFn_d1_z_noe_semiz(d1,d2,aprime,a,semiz,z,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit);
TemptationFn_e=@(d1,d2,aprime,a,semiz,e,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit) GPTemptationFn_d1_noz_e_semiz(d1,d2,aprime,a,semiz,e,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit);
TemptationFn_ze=@(d1,d2,aprime,a,semiz,z,e,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit) GPTemptationFn_d1_z_e_semiz(d1,d2,aprime,a,semiz,z,e,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit);
TemptationFn_none=@(d1,d2,aprime,a,semiz,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit) GPTemptationFn_d1_noz_noe_semiz(d1,d2,aprime,a,semiz,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit);
...
fprintf('Cross test 1 (semiz z_noe, plain): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(V1(:)-Vstd1(:))))
fprintf('Cross test 1 (semiz z_noe, plain): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(P1(:)-Pstd1(:))))
...
fprintf('Cross test 1 (semiz z_noe, DC): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(V2(:)-Vstd2(:))))
fprintf('Cross test 1 (semiz z_noe, DC): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(P2(:)-Pstd2(:))))
...
fprintf('Cross test 1 (semiz z_noe, GI): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(V3(:)-Vstd3(:))))
fprintf('Cross test 1 (semiz z_noe, GI): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(P3(:)-Pstd3(:))))
...
fprintf('Cross test 1 (semiz z_noe, DC+GI): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(V4(:)-Vstd4(:))))
fprintf('Cross test 1 (semiz z_noe, DC+GI): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(P4(:)-Pstd4(:))))
...
fprintf('Cross test 1 (semiz noz_e, plain): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(V1e(:)-Vstd(:))))
fprintf('Cross test 1 (semiz noz_e, plain): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(P1e(:)-Pstd(:))))
...
fprintf('Cross test 1 (semiz noz_e, DC): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(V1e(:)-Vstd(:))))
fprintf('Cross test 1 (semiz noz_e, DC): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(P1e(:)-Pstd(:))))
...
fprintf('Cross test 1 (semiz noz_e, GI): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(V1e(:)-Vstd(:))))
fprintf('Cross test 1 (semiz noz_e, GI): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(P1e(:)-Pstd(:))))
...
fprintf('Cross test 1 (semiz noz_e, DC+GI): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(V1e(:)-Vstd(:))))
fprintf('Cross test 1 (semiz noz_e, DC+GI): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(P1e(:)-Pstd(:))))
...
fprintf('Cross test 1 (semiz z_e, plain): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(V1ze(:)-Vstd(:))))
fprintf('Cross test 1 (semiz z_e, plain): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(P1ze(:)-Pstd(:))))
...
fprintf('Cross test 1 (semiz z_e, DC): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(V1ze(:)-Vstd(:))))
fprintf('Cross test 1 (semiz z_e, DC): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(P1ze(:)-Pstd(:))))
...
fprintf('Cross test 1 (semiz z_e, GI): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(V1ze(:)-Vstd(:))))
fprintf('Cross test 1 (semiz z_e, GI): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(P1ze(:)-Pstd(:))))
...
fprintf('Cross test 1 (semiz z_e, DC+GI): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(V1ze(:)-Vstd(:))))
fprintf('Cross test 1 (semiz z_e, DC+GI): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(P1ze(:)-Pstd(:))))
...
fprintf('Cross test 1 (semiz noz_noe, plain): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(V1n(:)-Vstd(:))))
fprintf('Cross test 1 (semiz noz_noe, plain): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(P1n(:)-Pstd(:))))
...
fprintf('Cross test 1 (semiz noz_noe, DC): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(V1n(:)-Vstd(:))))
fprintf('Cross test 1 (semiz noz_noe, DC): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(P1n(:)-Pstd(:))))
...
fprintf('Cross test 1 (semiz noz_noe, GI): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(V1n(:)-Vstd(:))))
fprintf('Cross test 1 (semiz noz_noe, GI): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(P1n(:)-Pstd(:))))
...
fprintf('Cross test 1 (semiz noz_noe, DC+GI): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(V1n(:)-Vstd(:))))
fprintf('Cross test 1 (semiz noz_noe, DC+GI): lambda=0 equals standard, this should be zero: %.3e \n',max(abs(P1n(:)-Pstd(:))))
...
% The nosemiz z-side (uses the GPU-validated nosemiz GP family; with d, z plays the semiz role)
ReturnFn_zside=@(d,aprime,a,z,r,w,kappa_j,sigma,eta,varphi,agej,Jr,pension) ReturnFn_d_z_noe_nosemiz(d,aprime,a,z,r,w,kappa_j,sigma,eta,varphi,agej,Jr,pension);
TemptationFn_zside=@(d,aprime,a,z,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension) GPTemptationFn_d_z_noe_nosemiz(d,aprime,a,z,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension);
...
for tier_c=1:4
    ...
    fprintf('Cross test 2 (semiz just-a-markov, %s): V vs nosemiz GP, this should be zero: %.3e \n',tiername,max(abs(Vs(:)-Vz(:))))
    % Drop the d2 row of the semiz Policy (row 2 here: rows are (d1,d2,aprime,...)), then compare
    Psshort=Ps([1,3:size(Ps,1)],:,:,:);
    fprintf('Cross test 2 (semiz just-a-markov, %s): Policy (without d2 row) vs nosemiz GP, this should be zero: %.3e \n',tiername,max(abs(Psshort(:)-Pz(:))))
end
...
fprintf('Cross test 3 (semiz z_noe, plain): first half vs V_Jplus1 solve, this should be zero: %.3e \n',max(abs(temp(:))))
...
temp=reshape(V5a,[],N_j); temp=temp(:,1:half)-reshape(V5c,[],Njs);
fprintf('Cross test 3 (semiz z_noe, DC): first half vs V_Jplus1 solve, this should be zero: %.3e \n',max(abs(temp(:))))
temp=reshape(P5a,[],N_j); temp=temp(:,1:half)-reshape(P5c,[],Njs);
fprintf('Cross test 3 (semiz z_noe, DC): first half vs V_Jplus1 solve, this should be zero: %.3e \n',max(abs(temp(:))))
% GI tier
...
temp=reshape(V5a,[],N_j); temp=temp(:,1:half)-reshape(V5c,[],Njs);
fprintf('Cross test 3 (semiz z_noe, GI): first half vs V_Jplus1 solve, this should be zero: %.3e \n',max(abs(temp(:))))
temp=reshape(P5a,[],N_j); temp=temp(:,1:half)-reshape(P5c,[],Njs);
fprintf('Cross test 3 (semiz z_noe, GI): first half vs V_Jplus1 solve, this should be zero: %.3e \n',max(abs(temp(:))))
% DC+GI tier
...
temp=reshape(V5a,[],N_j); temp=temp(:,1:half)-reshape(V5c,[],Njs);
fprintf('Cross test 3 (semiz z_noe, DC+GI): first half vs V_Jplus1 solve, this should be zero: %.3e \n',max(abs(temp(:))))
temp=reshape(P5a,[],N_j); temp=temp(:,1:half)-reshape(P5c,[],Njs);
fprintf('Cross test 3 (semiz z_noe, DC+GI): first half vs V_Jplus1 solve, this should be zero: %.3e \n',max(abs(temp(:))))
...
temp=max(Vgp(:)-Vstd1(:));
fprintf('Cross test 4 (semiz z_noe, plain): max(V_GP - V_standard), this should be weakly negative: %.3e \n',temp)
